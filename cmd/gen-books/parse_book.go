...
// make sure chapter/section ids within the book are unique, so that we can generate stable urls
func ensureUniqueIds(book *Book) {
	chapterIds := make(map[string]*Chapter)
	sectionIds := make(map[string]*Section)
	for _, c := range book.Chapters {
		...
		for _, s := range c.Sections {
			if sec, ok := sectionIds[s.ID]; ok {
				fmt.Printf("Duplicate section id: '%s', in:\n", s.ID)
				fmt.Printf("Section '%s', file: '%s'\n", s.Title, s.sourceFilePath)
				fmt.Printf("Section '%s', file: '%s'\n", sec.Title, sec.sourceFilePath)
				os.Exit(1)
			}
			sectionIds[s.ID] = s
		}
